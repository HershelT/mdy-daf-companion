...
export function summarizeDailyStats(record) {
    ...
    return {
        ...
        watchToCodingRatio: record.codingSeconds > 0 ? Number((record.watchedSeconds / record.codingSeconds).toFixed(2)) : null
    };
}
export function getTodayStatsSummary(now = new Date()) {
...
export function getDashboardStatsSummary(now = new Date()) {
    const paths = resolveRuntimePaths();
    const config = loadConfig(paths);
    const date = civilDateInTimezone(now, config.timezone);
