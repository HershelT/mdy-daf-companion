...
export class YouTubeChannelPageCandidateProvider implements CandidateProvider {
  constructor(
    private readonly url = MDY_YOUTUBE_VIDEOS_URL,
    private readonly fetchImpl: typeof fetch = fetch
  ) {}

  async getCandidates(_query: DafYomiRef): Promise<VideoCandidate[]> {
...
export function extractYouTubeChannelCandidates(html: string): VideoCandidate[] {
  ...
  while ((match = rendererPattern.exec(html))) {
    ...
    const publishedText = firstMatch(
      nearby,
    ...
    const lengthText =
      firstMatch(
        nearby,
        /"lengthText"\s*:\s*\{\s*"accessibility"\s*:\s*\{\s*"accessibilityData"\s*:\s*\{\s*"label"\s*:\s*"([^"]+)"/
      ) || durationLabelNearVideoId(html, videoId);
...
function extractLooseCandidates(html: string): VideoCandidate[] {
  const candidates = new Map<string, VideoCandidate>();
...
function parseDurationLabel(label: string): number | undefined {
  ...
  for (const value of values) {
    ...
    if (unit === "hour") seconds += amount * 3600;
    if (unit === "minute") seconds += amount * 60;
    if (unit === "second") seconds += amount;
  }
  return seconds;
}

function durationLabelNearVideoId(html: string, videoId: string | null): string | null {
  if (!videoId) {
    return null;
  }
...
function decodeJsonString(value: string): string {
  return value.replace(/\\u([\dA-Fa-f]{4})/g, (_, code: string) =>
