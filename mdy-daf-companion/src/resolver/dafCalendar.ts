...
import type { DafYomiRef } from "./types.js";

export interface DafCalendar {
  getDafForDate(date: string): Promise<DafYomiRef>;
}
...
export class HebcalDafCalendar implements DafCalendar {
  constructor(private readonly fetchImpl: typeof fetch = fetch) {}

  async getDafForDate(date: string): Promise<DafYomiRef> {
    const url = `https://www.hebcal.com/hebcal?cfg=json&v=1&F=on&start=${encodeURIComponent(
      date
    ...
    return parseHebcalDafResponse((await response.json()) as HebcalResponse, date);
  }
}

export function parseHebcalDafResponse(response: HebcalResponse, requestedDate: string): DafYomiRef {
