import { loadConfig, saveConfig, type AppConfig } from "../core/config.js";
import { resolveRuntimePaths } from "../core/paths.js";

export function applySetupOptions(args: string[]): AppConfig {
...
export function formatSetupSummary(config: AppConfig): string {
  return [
...
function setString(args: string[], name: string, setter: (value: string) => void): void {
...
function setBoolean(args: string[], name: string, setter: (value: boolean) => void): void {
  const index = args.indexOf(name);
  if (index >= 0) {
    const value = args[index + 1];
