---
description: Resolve or explain which MDY Daf Yomi shiur should be played for a date, daf, language, or format preference.
---

# Resolve Daf Shiur

Use this skill when the user asks which Rabbi Eli Stefansky or MDY Daf Yomi shiur should play.

Resolution order:

1. Determine the target date, timezone, masechta, and daf.
2. Apply user preferences for language and format.
3. Prefer official MDY app/site metadata when available.
4. Use official MDY YouTube channel metadata when needed.
5. Do not choose the newest upload blindly. Match daf, language, and format.
6. If confidence is low, say what is uncertain and offer the top candidates.

Never download, mirror, or redistribute the video.

